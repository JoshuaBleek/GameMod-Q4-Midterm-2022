...
//Bleeker Addtion
int Benjamins; //benjamins as in $100 bills lol
float spawnQuantity;
int currentSpawn;
int previousSpawn;
//Bleeker End
...
//Bleeker Addition Start

void idPlayer::spawnGrunt(float num) { //func spawn grunts in waves
	int enemies = (int)num;
	for (int i = 0; i < enemies; i++) {
		cmdSystem->BufferCommandText(CMD_EXEC_APPEND, va("spawn monster_grunt\n"));
	}
}
void idPlayer::spawnWaves() {

	int currentTime = gameLocal.time;
	if (currentTime - previousSpawn >= 5000) {
		spawnGrunt(spawnQuantity);
		gameLocal.Printf("Spawnning!");
		previousSpawn = currentTime;
	}
	else if (previousSpawn == 0)
		spawnGrunt(spawnQuantity);
	else
		gameLocal.Printf("Not spawning yet.\n");
	spawnQuantity *= 1.5;
}
//Bleeker Addition End
